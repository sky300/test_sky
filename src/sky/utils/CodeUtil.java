...
public class CodeUtil {
    ...
    public static String formatAmount(String s) {
        ...
        if (j < i - 2)
            if (flag)
                return (new StringBuilder()).append(s1).append(s.substring(j, i - 2)).append(".")
                        .append(s.substring(i - 2)).toString();
            else
                return (new StringBuilder()).append(s.substring(j, i - 2)).append(".").append(s.substring(i - 2))
                        .toString();
        if (flag)
    ...
    public static boolean isHexString(String s) {
        ...
        if (abyte0.length % 2 != 0)
            return false;
        for (int i = 0; i < abyte0.length; i++)
            if ((abyte0[i] < 48 || abyte0[i] > 57) && (abyte0[i] < 97 || abyte0[i] > 102)
                    && (abyte0[i] < 65 || abyte0[i] > 70))
                return false;
